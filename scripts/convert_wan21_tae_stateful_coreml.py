#!/usr/bin/env python3
"""Convert Wan 2.1 decoder-only TAE to CoreML stateful chunked decoder."""
import argparse
import subprocess
...
from taehv import Clamp, MemBlock, TAEHV, TGrow, conv


class DecoderOnlyTAEHV(nn.Module):
    """Decoder-only fork matching TAEHV decoder architecture for Wan 2.1."""
...
def parse_decoder_time_upscale(value: str) -> tuple[bool, bool, bool]:
    ...
    if len(s) != 3 or any(ch not in "01" for ch in s):
        raise argparse.ArgumentTypeError(
            f"Invalid decoder-time-upscale '{value}'. Use a 3-bit string like 000 or 111."
        )
...
class Wan21TemporalDecoderWrapper(nn.Module):
    """NTHWC latent -> NTHWC RGB for full decode (trimmed)."""
...
class Wan21StatefulChunkDecoderWrapper(nn.Module):
...
def run_stateful_chunked_decode_torch(
    wrapper: Wan21StatefulChunkDecoderWrapper,
...
def check_equivalence_to_taehv(
    checkpoint_path: str,
    wrapper: Wan21TemporalDecoderWrapper,
    latent_t: int,
    latent_hw: int,
    latent_channels: int,
    decoder_time_upscale: tuple[bool, bool, bool],
...
def check_stateful_equivalence(
    full_wrapper: Wan21TemporalDecoderWrapper,
    stateful_wrapper: Wan21StatefulChunkDecoderWrapper,
    latent_hw: int,
    latent_channels: int,
    chunk_t: int,
    state_shapes_chw: list[tuple[int, int, int]],
    ...
    with torch.no_grad():
        for t_total in lengths:
            for seed in (0, 1):
                torch.manual_seed(2000 + 17 * t_total + seed)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--out-dir", default="coreml_out/wan21_decoder_stateful_nthwc", help="Output directory")
    p.add_argument("--latent-hw", type=int, default=96, help="Latent H/W (e.g. 96 for 768 output)")
    p.add_argument("--chunk-t", type=int, default=2, help="Fixed latent frame count per CoreML call")
    p.add_argument("--latent-channels", type=int, default=16)
    p.add_argument(
        "--decoder-time-upscale",
        type=parse_decoder_time_upscale,
        default=(False, False, False),
        help="3-bit temporal upscale flags for decoder TGrow blocks (e.g. 000 or 111).",
    )
    ...
    p.add_argument(
        "--trace-fp16",
        action="store_true",
        help="Trace the PyTorch wrapper in fp16 to reduce host RAM during large-shape conversion.",
    )
...
def main() -> None:
    ...
    full_wrapper = Wan21TemporalDecoderWrapper(dec_model).eval()
    wrapper = Wan21StatefulChunkDecoderWrapper(dec_model).eval()

    if not args.skip_equivalence_check:
        check_equivalence_to_taehv(
    ...
    trace_dtype = torch.float16 if args.trace_fp16 else torch.float32
    if args.trace_fp16:
        wrapper = wrapper.half()
    example_latent = torch.randn(
        1, args.chunk_t, args.latent_hw, args.latent_hw, args.latent_channels, dtype=trace_dtype
    )
    ...
    stem = f"wan21_decoder_stateful_nthwc_chunk{args.chunk_t}_hw{args.latent_hw}_dtu{dtu_tag}"
    if args.convert_to == "mlprogram":
